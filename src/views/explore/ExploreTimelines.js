...
import ExploreTimelineCard from 'src/components/ExploreTimelineCards'

const Timelines = () => {
  const [page, setPage] = useState(1)
  const [count, setCount] = useState(2)
  const apiURL = "timelines/timelines/discover/?page=" + page.toString();
  ...
  return (<>
    {timelines != null ? (<><CRow>
      {timelines && timelines.map((time, index) => (
        <ExploreTimelineCard key={time.id} time={time} />
      ))}
    </CRow>
